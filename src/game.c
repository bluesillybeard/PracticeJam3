...
bool practiceJam3_game_step(PracticeJam3State* state) {
    ...
    // This is actually a feature, because it means I can define the outer bounds of a level with a big AABB
    ...
    for(size_t index = 0; index < scene1.numColliders; ++index){
        ...
        collisionFace = gameCollide((GameCollider){.AABB={this->playerX, this->playerY, 1, 1}}, movex, movey, col, 0, 0, &collisionTime);
        collisionTime -= collisionTimeBuffer;
        if(collisionTime < 0) collisionTime = 0;
        if(collisionFace){
            SDL_Log("Collided on face %i\n", collisionFace);
